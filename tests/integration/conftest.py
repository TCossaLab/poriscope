...
def sample_chimera(tmp_path):
    """
    Locate a small Chimera .log + matching .json pair under tests/data/,
    copy them into the test's tmp_path, and return their locations.
    """
    # Adjusted to point to tests/data/ from tests/integration/
    data_dir = Path(__file__).parent.parent / "data"

    if not data_dir.exists():
        pytest.skip("Missing folder: tests/data/")
    ...
    if not (src_log and src_json):
        pytest.skip(
            "No matching Chimera pair found in tests/data/ "
...
def sample_events_db(tmp_path) -> str:
    ...
    Expects a single file at:
      tests/data/events.sqlite3
    """
    src = Path(__file__).parent.parent / "data" / "events.sqlite3"
    if not src.exists() or not src.is_file():
        pytest.skip("Missing tests/data/events.sqlite3")
...
def sample_metadata_db() -> str:
    ...
    Expects the file to exist at tests/data/DB.db.
    """
    p = Path(__file__).parent.parent / "data" / "DB.db"
    if not p.exists() or not p.is_file():
        pytest.skip("Missing tests/data/DB.db")
    return str(p)

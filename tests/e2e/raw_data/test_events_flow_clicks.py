...
def test_commit_events_writes_exact_schema(qtbot, monkeypatch, tmp_path, events_found):
    ...
    # NOTE: wait for the committed row count, not just for the tables. sqlite3
    # creates the database file the instant the writer opens it, and
    # SQLiteEventWriter commits its CREATE TABLE statements on their own
    # connection well before it commits the event rows on a second, longer-
    # lived connection - so waiting only on out_db.exists() or on table
    # presence can return while zero events have actually been written, which
    # made the assertion below fail intermittently in full-suite runs and
    # pass in isolation.
    qtbot.waitUntil(
        lambda: sqlite_has_tables(out_db, {"channels", "events", "columns"})
        and sqlite_row_count(out_db, "events") == ds.num_events,

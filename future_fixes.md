...
- **Condition qualification rewrites SQL string literals.**
  ...
  `e.sequence = 's.sublevel_current'` - valid SQL that returns the wrong rows with no
  error. It also emits `exp.voltage` into a query whose FROM clause has no `experiments`
  join unless an experiments column was *selected* (`:854`), so a legitimate `voltage > 50`
  filter fails as "Invalid query".

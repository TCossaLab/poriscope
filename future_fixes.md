...
2. `analysistabs/` GUI family - 5 tab triads + `*controls.py` mixins + `walkthrough*` (the largest single chunk of remaining volume). **Done** (2026-08-24): all 22 files annotated, one tab family per commit, in the order shared-utils -> Clustering -> EventAnalysis -> RawData -> Metadata -> Protein. Every file passes the AST completeness scan (no parameter or return left unannotated) and `pydoclint --arg-type-hints-in-signature=True` reports zero `DOC104`-`DOC107` anywhere under `analysistabs/`. `pre-commit run --all-files`, `test_plugin_compliance.py` (71 cases), and `pytest -m "not e2e and not slow"` (2615 passed, 2 skipped) were all run green after each tab before committing it. `.pydoclint-baseline.txt` was regenerated per tab; every regeneration was a pure addition of `DOC108` policy nags with nothing removed or altered.
   ...
   **Still flagged, not fixed:** `MetadataView.hist_data` and `ProteinView.hist_data` hold three different shapes depending on which plot path filled them (widened to `List[Any]` with a comment; unifying them is a real refactor); and `on_button_clicked` in both `rawdatacontrols` and `eventAnalysisControls` ends with `button_mapping.get(button_type, lambda: None).setChecked(False)`, which would raise on an unmapped button type - mypy does not catch this one, because `.get()`'s default widens the inferred type, so it needs a human decision rather than a gate. The one remaining `# type: ignore` under `analysistabs/` is the `moveEvent` assignment covered by step 7 below.
...
7. Resolve the last `# type: ignore` under `analysistabs/`: `WalkthroughMixin._run_next_walkthrough_step` monkey-patches the dialog's Qt virtual with `self.walkthrough_dialog.moveEvent = self._reposition_dialog`, which mypy reports as `method-assign`. Deferred deliberately, because it is the only item in this pass that cannot be fixed without a (small) behaviour change, and because the obvious assumption about it is wrong.

   **Verified, do not re-derive:** the patch is *live*, not dead code. `StepDialog` has no class-level `moveEvent`, and PySide6 normally dispatches Qt virtuals through the type, so the natural conclusion is that Qt never calls this. A direct probe (assign `moveEvent` on a `QDialog` instance after `show()`, then `move()` and `processEvents()`) shows Shiboken *does* honour the instance attribute - it fired once, versus three times for an equivalent class-level override. So deleting the line would remove real behaviour.

   **The fix** is to give `StepDialog` a real hook instead of patching it from outside: an `on_move: Optional[Callable[[QMoveEvent], None]]` attribute plus a class-level `def moveEvent(self, event): super().moveEvent(event); ...` that invokes it, with the mixin assigning `self.walkthrough_dialog.on_move = self._reposition_dialog`. Two caveats that make this worth doing deliberately rather than in a typing sweep: the instance attribute currently *replaces* the virtual outright, so the base `QWidget::moveEvent` would begin running (a no-op, but a real difference); and the differing call counts above suggest the hook would fire on more move events than the patch does today. `_reposition_dialog` is idempotent and the 300 ms `reposition_timer` already re-runs the same positioning continuously, so extra calls should be harmless - but that should be confirmed against the running app, not just the test suite.

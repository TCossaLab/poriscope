...
* **New Dev Tooling: Type annotations for data plugins**
    ...
    * **Flagged for later, not fixed (docstring/type-hint-only pass, no logic changes):** `SQLiteEventWriter._write_data` calls `int()` directly on `start_sample`/`padding_before`/`padding_after`, but the shared `MetaWriter` contract types all three `Optional[int]` - latent unless one is ever actually `None` given current call patterns. `SQLiteEventWriter._rescale_data_to_adc`'s `dtype` default is the string `"u2"` where the base contract declares `type`; its own docstring already notes the parameter is unused by this writer.

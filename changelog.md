...
* **New Dev Tooling: `pydoclint`**
    ...
    * Cleared the remaining non-`DOC108` baseline entries for `walkthrough.py`, `MetaFilter.py`, and `MetaEventLoader.py` (445 -> 431 remaining lines):
        * `MetaFilter`'s class docstring opened with a bare `:ref:`MetaFilter`` role at column 0 - the same parser-breaking pattern already fixed for `MetaReader.py` - which silently prevented its `Attributes:` block from being recognized at all; reworded the opening line to `This class, :ref:`MetaFilter`, is...` to match the working pattern used elsewhere.
        * `IntroDialog`'s class docstring mixed a Google-style `Attributes:` block with Sphinx `:param:` fields for its constructor args in the same docstring, which pydoclint's parser doesn't reliably recognize together; switched to the Sphinx-correct `.. attribute :: name` / `:type:` directive for documenting its `start_walkthrough` signal, which coexists correctly with the constructor `:param:` fields.
        * `MetaEventLoader`: reordered `get_empty_settings`/`load_event`/`get_num_events`/`get_samplerate`/`get_event_generator`/`get_channels`'s docstrings so their `:param:`/`:return:`/`:rtype:` field lists come after the prose instead of interrupting it (the same prose-swallowed-into-`:rtype:` parser quirk seen before), and added `load_event`'s undocumented `data_filter` parameter.
    * Swept the codebase for bare `except:` blocks and narrowed each to `except Exception:` (checked in, since this is a code change and not purely docstring/type-hints, though behavior-neutral in every case found - each one immediately re-`raise`s, so the only observable difference would be for `BaseException`-only cases like `SystemExit`/`KeyboardInterrupt`, which propagate either way since the clause unconditionally re-raises): `DictDialog.get_input_file`/`get_output_file` (`poriscope/views/widgets/dict_dialog_widget.py`, this is what motivated the sweep - documenting `:raises Exception:` there previously tripped DOC503 since a bare re-raise has no statically-inferable type, while removing the `Raises` section instead tripped DOC501), `EventWorker.run` (`poriscope/utils/EventWorker.py`), and `MetaWriter._commit_events` (`poriscope/utils/MetaWriter.py`).

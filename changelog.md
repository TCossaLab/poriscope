...
* **A single-population prominence dataset is now thresholded and split rather than dumped wholesale into one class**
    * Previously `_classify_peak_prominences` reported `threshold: None` and marked every eligible peak class 1 when only one population was found - the behaviour its own docstring had specified since before this release. That answers "which of two populations is this peak in" with "there is only one", but it discards the separate and still-answerable question of which peaks are *too prominent for the population that was found*.
    * **The threshold is now the first local minimum of the smoothing spline above `2 * mean1 - 2 * std1`**, where `mean1`/`std1` are the lower fitted component's. Peaks below it are class 0, at or above it class 1. On a reconstruction of a real 6261-peak single-population dataset this put the threshold at 3414.6 pA against a floor of 3372.0, splitting 6056 (96.7%) / 205 (3.3%).
    * **The FIRST local minimum above the floor, not the deepest, and the difference is not cosmetic.** Past the bulk of the data the spline is fitting counting noise on near-empty bins, so it wiggles, and every wiggle is a local minimum. That reconstruction had 43 local minima in total, 30 of them above the floor; the deepest sat at 5165 pA with a spline value of **-0.29** - below zero counts, purely noise - and would have cut off 0.7% of the data at an essentially arbitrary point. The first one above the floor lands just past the floor, which is the boundary the floor was pointing at in the first place. This is the same reason the *between-centres* valley search still takes the deepest: there the search is bracketed by two real modes, so every candidate in range is a genuine feature.
    * **This replaces the old `"fallback"` path as the primary route, rather than removing it.** `_threshold_between_populations` now tries, in order: a valley between the two fitted centres (`"spline_valley"`), the first local minimum above the floor (`"spline_valley_above_floor"`, new), the first raw data point above the floor (`"fallback"`), and the midpoint of the two means (`"fallback_degenerate"`). Both spline-derived methods enable peeling; the two fallbacks do not, since those thresholds are not read off any feature of the curve.
    * **The between-centres valley search is now skipped outright when the fit reports one population, and that fix was needed for any of the above to run at all.** On the first attempt the new rule never fired: a real 6261-peak dataset came back with a 2045 pA threshold, far *below* its own 3262 pA floor. The cause is that the between-centres search returns first, and on single-population data it still had a bracket to search - the two centres were 1787 and 2088, both on the same mode, 301 pA apart. Beside a peak of ~450 counts per bin, Poisson noise alone is about +/-21 counts, which is ample to put a local minimum somewhere in that window, and the search duly found one and returned it. The bracket separates nothing on unimodal data, so there is no correct answer inside it and no threshold on how deep a valley must be that would fix this; the search simply must not run. `_threshold_between_populations` takes a `one_population` argument for this.
    * **The higher Gaussian is re-fit above the threshold on this path too, small amplitude and all.** Leaving it where the joint fit put it - on top of the lower component - draws a second population on the plot that the classifier has just reported it did not find. Peeling now runs on the single-population path as well, so the higher component describes only the sparse region above the threshold: on the reconstruction it comes back at mu 3414.6, std 894.6, amplitude **7.7** against the lower component's 405. A small amplitude is the correct answer rather than a failed fit - it says the region above the threshold is sparsely populated, which is what the plot should show. This also widens the lower component (mu 1796.8/std 110.8 from the joint fit to mu 1911.8/std 274.4 peeled) for the same reason it does on two-population data: with the higher component no longer overlapping it, it has to account for its own right shoulder.
    * **Deliberately scoped to peak prominence.** `_classify_folded_unfolded` and `_classify_translocation_direction` continue to decline on single-population data: "folded" and "forward" are claims about a second population that was not found, whereas "more prominent than this population accounts for" is not. Translocation direction additionally could not use this rule safely even if wanted - its values are `log10(ECD ratio)` and can be negative, so `2 * mean - 2 * std` can land below the entire dataset and make the threshold arbitrary. Both classifiers still get the improved threshold computed and drawn on their plots; they just do not act on it. Asserted in the tests, which check that neither fabricates an event label on single-population input.

* **The two Gaussian components are now fit one per side of the valley, instead of both at once against the whole histogram**
    * **The joint fit lets the higher component be recruited to patch the lower population's un-modelled right shoulder.** Real prominence and blockage populations are right-skewed (measured skew +2.09 on the upper population of the dataset below), so a symmetric Gaussian on the lower population falls away faster than the data does and leaves a heavy residual at its right flank. Least squares reduces that residual by widening the higher component and sliding its mean down into the shoulder - which lowers the total residual while making the higher component describe neither population. This is visible on a real 12096-peak prominence run as a broad red curve peaking around 2900 when the smoothing spline plainly puts the second mode near 3300.
    * **New `_peel_components_at_valley`, called from `fit_threshold`.** Splits the histogram at the spline valley and fits a single Gaussian to each side, so neither component can borrow the other's data to reduce its own residual. Measured on a reconstruction of that dataset - built by sampling the histogram profile read off the plot, which reproduces the reported `std2=758` as 755 - the higher component's mean moved from 114 pA off the data's real mode to 10 pA off it, its width from 755 to 621, and its model-vs-data RMS over the upper population from 19.4 to 16.4.
    * **It does not touch the threshold, and there is no circularity.** The threshold is read off the smoothing spline, which peeling does not change; the threshold locates the split, the split refines the parameters, and the parameters are never fed back into the threshold. Asserted directly in the tests: `fit_threshold`'s threshold is bit-identical to the one `_threshold_between_populations` returns from the joint fit alone.
    * **Peeling declines rather than guessing** when there is nothing to peel: on the two fallback threshold paths the threshold is not read off any feature of the curve, and `fallback_degenerate` in particular is the midpoint of two co-located means, so splitting there would carve one mode arbitrarily in half. It also declines - keeping the joint fit - when either side has fewer than 4 bins for a 3-parameter fit, when either single-Gaussian fit fails to converge, or when either peeled component collapses to within a bin width. The last two log at warning; the structural cases at debug. `n_components` is deliberately computed *before* peeling, from the joint fit, so the one-vs-two-population decision is unchanged by this. (Peeling was initially gated to two-population data as well; the entry above extends it to the single-population path, where the higher component describes the region above the threshold.)
    * **The trade-off, which is real and worth knowing: the lower component gets wider.** Peeled, it must account for its own right shoulder rather than offloading it, so it widens and its mean is pulled off the mode - measured on the same reconstruction, from mu 1890/std 261 to mu 1924/std 301, moving its mean from 74 pA off the lower mode to 108 pA off it. In exchange it describes its own population better overall (RMS on its own side of the valley improved from 132.8 to 108.0), and the higher component improved on every measure. A single symmetric Gaussian cannot be both centred on a skewed population's mode and cover its shoulder; the joint fit only appeared to manage it by spending the *other* component to do so. The `_classify_folded_unfolded` centre ratio the 1.7-2.3 check reads shifted 1.743 -> 1.767 on that data, well inside the band.
    * `fit_threshold` gained a `"params_method"` key recording `"peeled"` or `"joint"`. Nothing else changed shape: `"params"` is the same six-tuple in the same `(amp, mean, std)` per-component order, so the plotting code and all three classifiers consume it unchanged.
    * **Measured and rejected on the way here, since two of these were previously recommended in this changelog's own earlier entries or in conversation.** Poisson (`1/sqrt(count)`) weighting of `curve_fit` **alone makes the higher component substantially worse** - mean error 114 -> 246 pA, RMS 19.4 -> 27.1 - because 25 of the 74 fit bins hold 5 counts or fewer and a 1-count bin then carries ~32x the weight of the 1000-count peak bin, letting the far tail dominate the objective. It only helps when paired with aggressive tail trimming, and the pairing has a cliff in it (still bad trimming at the 99th percentile, good at the 98th), which is not a property worth shipping. Tail trimming on its own is near-neutral (114 -> 105). Bounding the higher mean above the valley does nothing, because it is already above the valley in both fits. A log-normal upper component beat a Gaussian by 24% RMS and remains the strongest option not taken, deferred because it breaks the six-tuple `params` contract.

* **`fit_threshold` now places the threshold at the valley between the two fitted populations, instead of the arithmetic midpoint of their means**
    * The midpoint ignores the histogram's actual shape. On unevenly-sized populations (e.g. 9000 events at one mean, 1000 at the other) the real valley sits well off-centre, pulled toward the smaller population - a synthetic 9:1 mix with means 200.0/498.9 has its valley at 287.4, not the midpoint's 349.5. The midpoint cuts into whichever population is smaller.
    * **New `_threshold_between_populations`, called from `fit_threshold`.** Fits a smoothing spline (`scipy.interpolate.make_smoothing_spline`, chosen specifically because it selects its own smoothing by generalized cross-validation - no smoothing factor to hand-tune per dataset) to the same histogram the double-Gaussian fit used, evaluates it on a fine grid between the two fitted means, and looks for a local minimum - a real dip in bin counts between the two modes. Where more than one shows up, the deepest is used. Local minima are found from sign changes in consecutive differences of the sampled curve rather than `scipy.signal.argrelmin`'s strict less-than-both-neighbours test, so a short flat run at the bottom of a shallow valley is still caught.
    * **Fallback when no valley exists**, which happens once the two populations overlap enough that the smoothed histogram is monotonic all the way from one mean to the other: the threshold becomes the first raw data point above `2 * mean1 - 2 * std1`, where `mean1`/`std1` belong to whichever fitted component has the lower mean. If no data point clears that floor either (observed on heavily-overlapping synthetic data whose spread doesn't reach it), this falls back once more to the plain midpoint rather than raising, logged as a warning either way (`"threshold_method"`: `"spline_valley"`, `"fallback"`, or `"fallback_degenerate"`).
    * **The `2 * mean1 - 2 * std1` fallback is shaped for `_classify_folded_unfolded` specifically**, where a folded carrier blocks roughly twice as deep as unfolded (the existing 1.7-2.3 ratio check), so `2 * mean1` approximates where the second population's mean should sit. Verified against synthetic data without that 2:1 relationship (peak-prominence- and translocation-direction-shaped inputs): the floor lands above every data point and the fallback degrades to the plain midpoint rather than picking something misleading - it does not fail, but it also adds nothing over the previous behaviour outside the folding context. `fit_threshold` is shared by all three classifiers and applies the same formula to all of them, as specified; flagged here rather than special-cased per caller.
    * **The dict key `fit_threshold` returns changed from `"midpoint"` to `"threshold"`**, since the value is no longer necessarily a midpoint of anything. All three `_classify_*` call sites and their local variable names were updated to match; nothing outside `fit_threshold`'s own dict was affected, since `_classification_results["threshold"]`, `_peak_prominence_classification_results["threshold"]` and `_translocation_direction_results["threshold"]` were already named `"threshold"` at the classifier-result level and are populated from `bt.get("threshold")` unchanged.
    * **The spline is drawn on all three classification plots**, via a new shared `_overlay_smoothing_spline` helper (one implementation, three call sites, following the `_jitter_degenerate_array` precedent). The spline is what chooses the threshold, so without it the threshold line had no visible justification on the plot - and in the fallback case, no visible explanation for why the valley search came up empty. `_threshold_between_populations` therefore now fits the spline first and unconditionally and returns it alongside the threshold, so it is available for plotting even on the paths that do not use it to place the threshold; `fit_threshold` passes it through as a new `"spline"` key.
        * **It is evaluated only over the bin *centres* it was fit on, never the plot's full x-range.** `make_smoothing_spline` returns a `BSpline`, which extrapolates without bound outside its knots, and the plotted histogram spans the bin *edges* - half a bin wider on each side. Asserted in the tests against both spans rather than left to inspection.
        * **It is drawn in the single-population case too**, where there is no threshold line to justify. That is the case it is most useful in: a plot reading "Selected populations: 1" with a single smooth hump and no dip is direct visual evidence for why no split was made.
        * Like the fitted-Gaussian overlays it sits beside, it is in the fit histogram's count units, which matches the plotted bars on the normal path where the plot reuses the fit's bin edges. On the pre-existing fallback path where the plot re-bins at 50 or 100 bins instead, it is on the same mismatched scale those overlays already are - noted rather than fixed, since fixing it for the spline alone would make the overlays inconsistent with each other.
        * **A smoothing spline can undershoot below zero across a wide empty gap between two well-separated populations**, and where it does, the minimum it reports sits at one end of that gap rather than mid-gap (observed: means 50.1/119.3 with an empty 70-92 gap, threshold placed at 73.0). This does not affect classification - every point in an empty gap classifies identically, verified as an exact 250/150 recovery of ground truth - and it only arises where there is no data to constrain the curve. It is left visible on the plot rather than clipped at zero, because clipping would misrepresent the curve the threshold was actually read off.
    * Verified against the real module: threshold sits strictly between the two fitted means on well-separated bimodal data and is measurably pulled off the naive midpoint on imbalanced populations; the fallback formula's "first point above the floor" was checked to match `np.sort(data)` exactly; all four plots were rendered for real and the spline line located on each by inspecting the axes' `Line2D` objects rather than by eye; and all three classifiers were re-run end-to-end and continue to reconcile (folded + unfolded == total, both prominence classes populated). `ruff`, `black` and `pydoclint` are clean on the file.

* **`PeakFinder`'s three classifiers now recognise a single-population dataset and stop force-fitting it as two**
    * Prompted by a real dataset (the same one behind the collapsed-fit entry below, post-fix): its second "population" turned out not to exist. The double-Gaussian fit no longer collapsed a component, but it still converged with two centres sitting on the same mode - a real result for a single decaying population, not a fitting bug, and the previous entry's own investigation had already reached that conclusion. Confirmed by direct inspection of the source data: it is genuinely one population. `fit_threshold` always returns two centres regardless, so nothing downstream could previously tell "real split" from "forced split" apart.
    * **`fit_threshold` gained a `"n_components"` key (`1` or `2`)**, computed from the same collapsed-component and centres-not-separated diagnostics `_fit_and_check_double_gaussian` already computed in the previous entry - they were logged as warnings there and are now acted on. `_fit_and_check_double_gaussian` itself now returns `(popt, one_population)` instead of just `popt`. The third diagnostic, unconstrained parameters, is deliberately *not* folded into this signal: it can also fire on a genuinely bimodal but small or heavily-overlapping dataset, which is a precision problem rather than a population-count one.
    * **A naive alternative was tried and rejected first: comparing a 1-component and a 2-component `GaussianMixture`'s BIC on the raw samples.** It works on clean synthetic data (Gaussian vs. two well-separated Gaussians) but **decisively picks 2 components on the real single-population dataset** - the one this whole feature exists for. Measured on the tuned reconstruction of that dataset (sharp core + decaying right shoulder, n=6233): BIC 88,198 at k=2 versus 90,936 at k=1, a drop past any conventional "decisive" threshold, and log-space fares no better (-16,771 vs -15,519). A skewed single population is closer, in likelihood terms, to two Gaussians than to one, and BIC's parameter-count penalty does not overcome that at this sample size. This is a structural property of comparing Gaussian components on non-Gaussian data, not a tuning problem, so the approach was replaced with the fit-diagnostic signal above rather than patched (e.g. with a BIC margin) - the diagnostics are already fit to *this* data's actual shape, not an idealised Gaussian standing in for it.
    * **The key name and the fallback behaviour for peak-prominence classification were not new decisions - the codebase already specified them and never implemented them.** `_classify_peak_prominences`'s docstring already read "If a single population is selected by BIC, all eligible peaks are as 1" and `report_channel_status` already read `prominence_stats.get("n_components", 0)` under a "Selected populations" label, both present before any work this release touched `PeakFinder.py`. Neither the BIC selection nor its producer ever existed; the docstring's behaviour is implemented here, and `"n_components"` is the key it and `report_channel_status` already expected. `_classify_peak_prominences` now reports every eligible peak as class 1, `threshold: None`, with `report_channel_status`'s existing display line finally populated.
    * **`_classify_folded_unfolded` and `_classify_translocation_direction` had no equivalent scaffolding, so they follow their own existing "can't tell, don't guess" precedent instead of peak-prominence's default-to-one-class behaviour.** Both already had a `centers.size < 2` branch for this case - `_classify_folded_unfolded` recording `{"error": "Could not find two distinct distributions"}`, `_classify_translocation_direction` recording `{"skipped": True, "reason": "insufficient centers"}` - but neither branch could ever actually fire, since `fit_threshold` always returns exactly two centres by construction. That dead code is left in place as a guard; the new `n_components < 2` check sits next to it and is what now actually triggers, with `_classify_folded_unfolded` also gaining a `"n_components"` key on its two-population success path so the count is visible either way. Neither method invents a folded/unfolded label or a forward/backward direction for a population it cannot resolve.
    * Verified against the real module with Qt stubbed: all three classifiers keep exact prior behaviour on clearly-bimodal synthetic data (`n_components == 2`, results unchanged), all three correctly decline on clean single-Gaussian synthetic data, and - the case that matters - all three correctly report `n_components == 1` on the tuned reconstruction of the real dataset that motivated this entry, where the rejected BIC approach reported `2`.

* **Fixed: the second Gaussian silently collapsing to zero width, leaving a single-Gaussian fit wearing two sets of parameters**
    * Symptom, seen on a real peak-prominence run: the legend read `Higher prominence fit (mu=2083.450, std=0.000)` and no red curve was visible. It was not missing from the plot - at `std=0` the component evaluates to exactly zero everywhere (and `nan` at its own mean), so it was being drawn flat along the x-axis. The reported threshold, 1941.974, was the midpoint between a real mean and the centre of a component contributing nothing, i.e. a cut at `mu1 + 0.82*sigma1` dressed up as a two-population boundary.
    * **Root cause was the stage-1 initial guess, not the bounds.** `find_peaks` was called with no `distance` argument, so on a histogram with one sharp mode and a smoothly decaying shoulder it returned two maxima **three bins apart** - the real peak (prominence 465) and a counting-noise wiggle on its right flank (prominence 69, against a 5%-of-maximum floor of 23 and a Poisson sigma of ~21 on that bin). Seeded with two Gaussians 152 pA apart, `curve_fit` converged to the nearest local minimum and squeezed the second component to a spike. It never explored the region where the actual higher-prominence population sits.
    * **Stage 2 would have got it right and never ran.** Its histogram-split guess structurally picks one seed per half (1835 and 2343, 508 pA apart) and converges to centres 1859.7 / 2475.6 with sigma 211 / 501. It was shadowed by stage 1 "succeeding".
    * **Fix: stage 1 now requires its two seeds to be at least one dominant-peak FWHM apart**, via a new `SEED_SEPARATION_FWHM = 1.0` class constant. The separation is derived from the dominant peak's own measured width rather than fixed in bins, so the criterion stays meaningful across binnings - two modes closer together than one linewidth are not resolved by that histogram anyway. When the constraint leaves fewer than two peaks, stage 1 declines and hands over to stage 2, which is the better-behaved path on this shape. Calibrated against synthetic data: stage 1 still fires and recovers ground truth on genuinely resolved modes (600 pA separation and up, e.g. 1806.7 / 3000.4 against a true 1800 / 3000), and correctly declines the 152 pA same-mode wiggle. The collapsing dataset now fits 1859.7 / 2475.6, and the previously-working bimodal dataset is unchanged at 1832.7 / 2807.0.
    * **`min_std` is now half a bin rather than zero**, in both stages, with `p0` clamped to match. This does *not* fix the collapse - floors of 0, 1, 25 and 50 were measured to converge identically, because the bound never binds - but at `std = 0` the model divides by zero, so it stops a `nan` reaching the plotted curve. Nothing narrower than the binning is meaningful for a binned fit.
    * **Three non-fatal diagnostics added to `_fit_and_check_double_gaussian`.** All three log and let the fit through, keeping the "failures stay visible" stance: a **collapsed component** (fitted sigma at or below one bin width); **centres that are not separated** (means closer than one FWHM of the narrower component - the same criterion stage 1 applies to its seeds, applied to the result); and **unconstrained parameters** (ProteinView's `perr > 10*|popt|` test, kept as a warning rather than the rejection it is there).
    * **This corrects a claim in the entry below.** That entry records that ProteinView's stripped statistical checks "would have accepted every bad fit anyway" - true of the dataset it was measured on, and wrong here. On this failure `perr` fires decisively (amp2 = 271 +/- 5.6e7) and the t-test gives p = 0.9996. The amplitude-ratio check is the one that genuinely misses it, scoring 0.64, because it is the width that degenerates and not the amplitude. Hence `perr` returning as a warning, and hence the separate centre-separation check, which covers the remaining gap: two components of comparable, non-degenerate width sitting on top of each other trip none of the others.

* **Experimental: `PeakFinder`'s three classifiers now use ProteinView's double-Gaussian fit instead of `bitthresh`**
    * `bitthresh` is **deleted**. `_classify_folded_unfolded`, `_classify_peak_prominences` and `_classify_translocation_direction` now call a new `fit_threshold`, which keeps `bitthresh`'s exact return shape (`midpoint`, `centers`, `hist`, `params`) so every downstream consumer and all the plotting code is unchanged. `pandas`, `warnings` and `OptimizeWarning` were only ever used by `bitthresh` and their imports are gone with it. This supersedes the consolidation `DECISIONS.md` deferred in favour of the owner's rewrite - this is that rewrite.
    * **Ported from `ProteinView`:** `_double_gaussian` (the model), `_fit_double_gaussian` (two-stage initial guess - two most prominent histogram peaks and their FWHM, falling back to splitting the histogram about its 5%-of-maximum support), and a reduced `_fit_and_check_double_gaussian`. `_double_gaussian` is deliberately left undecorated, as in ProteinView: `curve_fit` calls it hundreds of times per fit and a `@log` there floods the logfile. One latent bug in the original was corrected in passing: its histogram-split loop evaluates `amplitude[left_start]` before checking `left_start < n`, which would `IndexError` if no bin ever reached 5% of the maximum (unreachable for a real histogram, since the argmax bin always does).
    * **`params` changed order, and nothing catches that by arity.** `bitthresh`'s `dgfit` grouped by kind, `(a1, a2, u1, u2, w1, w2)`; ProteinView's model groups per component, `(amp1, mean1, std1, amp2, mean2, std2)`. Both are six-tuples, so the `len(params) != 6` guard added earlier in this release cannot see a swap - it would simply plot wrong curves. All three fitted-Gaussian overlay sites were converted to unpack in the new order, and every docstring states which order it means.
    * **Safeguards deliberately stripped, so the fit's real failure rate is observable rather than masked.** `_fit_and_check_double_gaussian` keeps only convergence checks (fit converged, covariance finite) and drops ProteinView's three statistical rejections: the `perr > 10*|popt|` test, the t-test on mean separation, and the 0.05 amplitude-ratio floor. `_classify_folded_unfolded` additionally loses its ECD 5th-95th percentile pre-filter and both re-fit fallbacks (the "centres too close, re-run on unfiltered data" branch and the "ratio outside 1.7-2.3, re-run on blockage-filtered data" branch). The centre ratio is still computed and reported, and is now **logged as a warning** when it falls outside 1.7-2.3 rather than triggering a re-fit - on synthetic single-population data that is what makes a degenerate 1.108-ratio split visible in the log. Measured against the cases that motivated stripping them, ProteinView's three statistical checks would have accepted every bad fit anyway (single population, 1% minority population, heavy overlap), so their removal costs nothing here.
    * **Also fixes the parked histogram low-end cut-off bug** recorded in `future_fixes.md`. With the ECD pre-filter gone, the histogram is built once from the full dataset and the same bin edges feed both the fit and the plot, so the plot can no longer be binned against edges derived from a different subset. `ecd_filtered_events` is retained in `_classification_results` (as 0) because `report_channel_status` reads it. `_classify_folded_unfolded` no longer takes `all_raw_ecds_array` and `_post_process_events` no longer builds it; the `raw_ecd > 0` event-validity check is unchanged. The percentile filters in the other two classifiers are untouched.
    * **New: a `MIN_FIT_BINS = 30` floor on the fit histogram, which `bitthresh` did not have and did not need.** Freedman-Diaconis sets bin width from the interquartile range, and on exactly the bimodal data these classifiers exist to separate, the two populations inflate the IQR and therefore the bin width, collapsing the histogram to a handful of bins. Measured on synthetic two-population data (means 300/600), FD alone gives 3 bins at n=60, 6 at n=300 and 7 at n=600 - against which a six-parameter double Gaussian is underdetermined. It failed outright below roughly 600 points and, worse, near 600-1000 could converge with *both* Gaussians sitting on the same mode (595.4 and 597.3) while passing every convergence check. `bitthresh` tolerated the same starved binning only because it fell back to raw peak locations whenever its fit failed. With the floor, the same data resolves correctly from n=60 to n=20000. The FD rule is otherwise unchanged, so bin count is not a confounding variable when comparing the two schemes.
    * **`fit_threshold` raises rather than falling back.** `bitthresh` returned centres taken from raw histogram peak locations when its fit failed, which made a failed fit indistinguishable from a successful one. A failed fit is now a `ValueError` that each classifier catches, logs, and records as an error result.
    * Verified end-to-end against the real module with Qt stubbed: on synthetic data all three classifiers recover ground truth exactly (folding 800/400 split at centres 297.4/598.9, ratio 2.014; prominences 250/150 at 49.97/120.11 against a true 50/120; direction 180 forward/120 backward), folded + unfolded counts reconcile with the event total, no plotting overlay is silently swallowed, and degenerate inputs (all-one-value, fewer than three points) fail loudly with a logged error rather than a silent skip. `ruff`, `pydoclint` and `black` are clean on everything touched.

* **Fixed: 14 silently-swallowed exceptions (`S110`/`S112`) in `PeakFinder.py`'s classifier**
    * A codebase-wide lint audit flagged 21 `try/except: pass`/`except: continue` sites; 7 outside `PeakFinder.py` were already fixed. These 14, all `except Exception:` swallowed with no logging, sit in `_classify_folded_unfolded`, `_classify_peak_prominences`, and `_classify_translocation_direction` - the folded/unfolded, peak-prominence, and translocation-direction classification decision paths.
    * **The most serious one:** `_classify_folded_unfolded` indexed `all_longest_levels_array[i]` inside `try: ... except Exception: continue`, so an out-of-range lookup dropped that event from `folded_count`/`unfolded_count` and skipped `update_event_metadata_post_processing` for it, with no log line - silently breaking the invariant that `folded_count + unfolded_count == total_events`. `all_event_info` and `all_longest_levels_array` are always built together, index-for-index, by the sole caller (`_post_process_events`), so the lookup cannot legitimately go out of range; replaced with an explicit `min(len(all_event_info), all_longest_levels_array.size)` bound and a `logger.warning` if the two ever disagree, rather than a `try` around a well-formed index.
    * **Two nested handlers** around the "re-run bitthresh on the unfiltered dataset if the fitted centers are suspiciously close together" fallback in `_classify_folded_unfolded` meant that fallback could silently not happen, with classification proceeding on the original centers as though the fallback had been considered and declined. Both now log at `warning` level with `exc_info=True` (kept broad - this is a best-effort improvement attempt with a well-defined "keep the original fit" fallback, not a condition narrow enough to enumerate as specific exception types) so a failed re-run is visible instead of indistinguishable from "didn't need to run."
    * **Three identical jitter guards** (one per classifier) around `np.nanmax(arr) - np.nanmin(arr)` meant to jitter a degenerate (empty/all-NaN/single-valued) array so histogramming works. `np.nanmax` raises `ValueError` on an empty array but only warns and returns `nan` on all-NaN (so the old `except Exception: pass` only ever actually caught the empty case, and would have hidden anything else raised in the block the same way). Replaced with an explicit `arr.size == 0 or np.all(np.isnan(arr))` check and a new shared `_jitter_degenerate_array` helper, deleting the `try` entirely.
    * **Seven plotting-overlay guards** (per-class histogram split, fitted-Gaussian curve overlay, and summary stats textbox, each duplicated across the three classifiers) now log at `debug` level with `exc_info=True` instead of swallowing silently - a reader was previously unable to tell "no class split found" from "the overlay crashed and nobody knows."
    * **The two fitted-Gaussian overlay sites** unpack a fixed six-tuple (`a1, a2, u1, u2, w1, w2 = params`) from `bitthresh`'s return value. A `ValueError` from a wrong arity was previously indistinguishable from any other plotting failure - exactly the failure mode that hid `fit_2_gauss`'s broken parameter count before it was deleted (see `DECISIONS.md`). Both sites now check `len(params) != 6` explicitly and log a `warning` naming the actual count before attempting the overlay, ahead of the owning developer's double-Gaussian rewrite.
    * One remaining site (assembling a debug-only log line in `_classify_folded_unfolded`) was cosmetic risk only; narrowed from `except Exception` to `(TypeError, IndexError, KeyError, AttributeError)` rather than adding logging, since the only content it guards is the log line itself.
    * No public API changes - all fourteen are inside private (`_`-prefixed) methods, none on a `Meta*` ABC. `ruff check --select S110,S112` now passes clean on `PeakFinder.py`; `ruff check`, `black --check`, and `pydoclint` all pass clean on the file. The project's own `pre-commit run mypy --all-files`/`pytest tests/unit/plugins/eventfitters` gates need to be run locally to fully confirm, since this integration environment cannot install the full PySide6/Qt project stack to run them directly - a scoped `mypy` pass against the file alone (ignoring unresolvable first-party imports) reported zero errors attributable to this change.

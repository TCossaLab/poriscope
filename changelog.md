...
* **New Dev Tooling: `pydoclint`**
    ...
    * **Flagged, not fixed:** `DictDialog.get_input_file`/`get_output_file` (`poriscope/views/widgets/dict_dialog_widget.py`) each wrap a call in a bare `except: raise`, which pydoclint can't reconcile either way: documenting `:raises Exception:` triggers DOC503 (a bare re-raise has no staticly-inferable exception type to match against), while removing the `Raises` section instead triggers DOC501 (a `raise` statement exists but is undocumented). The clean fix is changing `except:` to `except Exception:` so a nameable type exists, but that's a real (if near-behavior-neutral) code change rather than a docstring/type-hint one, so it was left as its pre-existing grandfathered `DOC503` violation pending review.

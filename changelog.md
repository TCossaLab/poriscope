...
* **Breaking: `MetaReader.load_data()` now raises on an out-of-bounds request instead of silently returning fewer samples than asked for**
    * Its own docstring has always promised `:raises ValueError: If start or end indices are out of bounds`, but the bounds check ran *after* `end_index`/`start_index` were already clamped down to `total_samples` - so the check could never see an overrun, and the promised exception was unreachable for exactly the case it names. A request extending past a channel's real end silently returned a shorter array instead, with nothing telling the caller their request was only partially satisfied.
    * Confirmed directly, not assumed: requesting one sample past a channel's actual end (`get_channel_length() + 1`) returned a shorter array with no exception before the fix, and now correctly raises `ValueError: Invalid data load request: ...`.
    * **This is a behavioural change for any caller that relied on the silent clamping**, deliberately or not. Traced every in-tree caller before making it: `continuous_read` and both `MetaEventFinder` call sites (`find_events`'s chunk loop, `get_single_event_data`'s padded-event window) already clamp their own requests to the channel's real length before calling `load_data` - `_get_padding_length` explicitly caps `padding_after` so `end + pa <= last_sample` - so none of them were relying on `load_data`'s internal clamp; it was dead defensive code masking the one case it was supposed to catch. Full suite (2799 passed) confirms no in-tree caller broke.
    * New regression test, `test_load_data_rejects_an_out_of_bounds_request` in `tests/unit/plugins/conformance/test_readers.py`, parametrized across all 7 readers. Verified it actually catches the regression it's for: reverted the fix, confirmed all 7 fail, restored it, confirmed all 7 pass again.
    * Found while scoping a plan for `future_fixes.md` block 7 (fuzz/malformed-input testing for readers) - not a mutation this fixture-mutating work needed to manufacture, since a plain over-long read request on an otherwise-valid file already reached it.

* **Fixed: `Basic_PeakFinder` crashed on a zero-width sublevel; it's now in the conformance suite**
    * `_populate_sublevel_metadata`'s `sublevel_max_deviation` computation had a ternary whose *both* arms slice `data[x:x]` (empty) when two consecutive sublevel-transition indices land on the same sample - reachable whenever `scipy.signal.find_peaks`'s interpolated half-height width crossing truncates to the peak's own index under noise. `np.max` on the resulting empty array raised `ValueError: zero-size array to reduction operation maximum which has no identity`, silently swallowed by `fit_events`'s per-event exception handling and recorded as a rejection rather than surfaced as a crash. Fixed by returning `0.0` for a zero-width sublevel - there is no data to take a deviation over, matching `sublevel_raw_ecd`'s existing `np.sum`-over-empty-slice precedent two lines above it, which already tolerates this case silently and correctly.
    * Found by, not before, un-skipping `Basic_PeakFinder` in `tests/unit/plugins/conformance/test_eventfitters.py`: `tests/synthetic_data/synthetic_events_db.py`'s `_build_event_trace` plants a single flat blockage per event, which a peak-based fitter has no local extremum to find. It now takes optional `sublevel_dip_pA`/`sublevel_dip_width_samples`, adding a smooth raised-cosine (Hann) taper on top of the flat blockage - deliberately not a rectangle: a rectangular dip's abrupt edges made the crash *more* frequent (edge ripple under noise resolving into two close-together peaks), not less, confirmed by trying it first. A second, separate `peaked_events_db_path` fixture uses the new knob; the original flat one, and the five fitters that already passed against it, are untouched.
    * `Basic_PeakFinder` now fits all 25 planted events and passes all four conformance checks - previously not just skipped but structurally unable to pass, since even a working fixture would have hit this crash.
    * `PeakFinder` stays skipped, with the reason corrected: not "no fixture" but "needs a fundamentally different one." Tried against the same peaked database under every setting attempted; it rejects all 25 events as "No Peaks Found" because its internal minimum peak prominence is derived from the blockage's own depth (~400 pA), not from any exposed setting, so a modest intra-event dip can never clear it. A fixture that could would need two comparably-deep levels, and would only reach the entry point to `PeakFinder`'s own downstream classification stages, unexplored here.
    * Verified: 108 passed/4 skipped in `tests/unit/plugins/conformance` (up from 104/8), the 66 existing `Basic_PeakFinder` unit tests unaffected, full suite 2792 passed/18 skipped/0 failed.

* **The settings-schema check is now reusable outside pytest, and enforced at commit time**
    * `poriscope/utils/settings_schema.py::validate_settings_schema()` is the static (schema-only) half of `tests/unit/plugins/test_settings_schema.py`'s checks - `Type`/`Value` presence, `Min <= Max`, `Options` element types - extracted so it no longer needs pytest to run. The test now imports it instead of duplicating the logic; behaviour is unchanged (36 passed, 12 skipped, same as before the extraction). The default-value half, which delegates to each plugin's own `_validate_param_types`/`_validate_param_ranges`, deliberately stays pytest-only: it needs a live instance, and reimplementing it would let it drift from the rules it enforces.
    * New `scripts/check_settings_schema.py` runs the static check standalone against every discovered plugin and exits non-zero on any violation, printed grouped by plugin. Silences the `@log` decorator's per-call DEBUG output first - left on, it buries the actual result under one line per method call across 24 plugins.
    * New local `settings-schema` pre-commit hook, scoped to `poriscope/plugins/**` (not all of `poriscope/`, since it only needs to re-run when a plugin's settings could have changed, and it is meant to run well ahead of the much more expensive conformance suite). Follows the same `language: system` pattern the `pydoclint` hook already uses, for the same reason: it needs the real project environment to import and instantiate plugins, not the dependency-free isolated venv the other pre-commit hooks run in.
    * Verified the gate actually catches something before landing it, not just that it runs clean: a `Min > Max` was deliberately injected into `BesselFilter`'s schema, the hook reported it with the plugin and parameter named and exited 1, then the injection was reverted and confirmed clean again.
    * `future_fixes.md`'s settings-schema block is now fully landed; its remaining open item (documenting `"Units"` as a real, if currently undeclared, schema field) is unchanged.
    * Contributor docs (`quality_control.rst`) updated: the settings-schema section now mentions the standalone script and hook, and both hook-enumeration lists (six hooks now, not five) gained the new entry.

* **`MetaReader` conformance lands: all 24 data plugins across all 8 `Meta*` families now run against real data**
    * `tests/unit/plugins/conformance/test_readers.py` covers the family the previous entry below left open. It needed five new synthetic on-disk formats under `tests/synthetic_data/` - `synthetic_binary.py` (`BinaryReader1X`, `SingleBinaryDecoder`), `synthetic_chimera_vc100.py` (`ChimeraReaderVC100`), a Chimera 2024-01 embedded-header writer added to `synthetic_chimera.py` (`ChimeraReader20240101`), and `synthetic_abf2.py` (`TCossaLabABFReader`, `LegacyElementsReader`) - since a reader's "recipe" is a file format, not settings. Each was reverse-engineered directly against the real parsing code and independently round-trip-verified before being wired into the suite. 36 tests, well under a second.
    * **Before this, only `ChimeraReader20240501` had ever been driven against real data** (by `tests/integration/flows` and `tests/e2e/raw_data`); the other 6 readers had signature compliance and settings-schema coverage only, never an actual file.
    * Four checks per reader: it agrees with the recording about channels, sample rate and length; `load_data` recovers the *planted baseline and event depth in picoamps*, not merely a plausible shape; `get_raw_dtype()` resolves to a real dtype and the raw-data path returns the same sample count as the normal one; `reset_channel`/`close_resources` are safe, including twice.
    * **The raw-dtype check in `future_fixes.md`'s original plan was invalid, and was corrected before being written into the suite rather than after.** `MetaReader.load_data` itself finishes its raw-data branch with `.astype(self.get_raw_dtype())` right before returning, so `load_data(raw_data=True).dtype == get_raw_dtype()` holds by construction for *every* reader - it is not a per-plugin invariant and would have passed even for a `_set_raw_dtype()` bearing no relationship to the file's actual on-disk type. A literal `raw*scale+offset` reconstruction was also considered and rejected: confirmed directly against `ChimeraReaderVC100`, whose `_convert_data` reinterprets the raw code through a bitmask/uint16 step that `(scale, offset)` alone doesn't capture, so that formula is not valid for the family in general. The check that survived is honest instead: `get_raw_dtype()` must resolve to a real dtype, and the raw path must return the same sample count as the normal one.
    * **The ABF2 writer needed the real parser reverse-engineered byte-by-byte** - `ABF2Header._read_abf2_header`'s section pointers, per-channel field layout, and the string-table indexing scheme it derives channel names/units from - rather than approximated, since a single wrong offset fails opaquely deep inside a `struct.unpack` call. Two mistakes were caught before landing, both by testing against the real reader immediately rather than trusting the derivation on paper: a struct format string one field short with a wrong type in the middle, and a dataset whose declared `channel` (0) didn't match what the reader would actually parse back out of the `CH003` token baked into the filename - the second is a real defect class (metadata disagreeing with what a consumer independently derives), not just a typo.
    * `LegacyElementsReader` and `TCossaLabABFReader` share the same on-disk ABF2 format but are not interchangeable fixtures: the former's `_get_configs` requires exactly 1 ADC channel and matches filenames via a trailing 4-digit token, the latter requires exactly 2 and a 12-digit-timestamp-plus-3-digit token - confirmed from `LegacyElementsReader`'s override before assuming one synthetic file could serve both.
    * Contributor docs updated: `quality_control.rst`'s conformance table gains a `MetaReader` row, its "what do I need to add" breakdown replaces "not yet covered" with the real answer (a new on-disk format needs a writer module; an existing one needs nothing), and a new tip warns against the raw-dtype tautology above.

* **Behavioural conformance extended to six more plugin families: 22 of 24 data plugins now run against real data**
    * `tests/unit/plugins/conformance/` gains `test_filters.py`, `test_eventfinders.py`, `test_eventloaders.py`, `test_dbloaders.py` and `test_writers.py`, each parametrised over discovered classes. With the fitters, seven of the eight `Meta*` families are covered; `MetaReader` is not, because it needs one synthetic writer per on-disk format and only Chimera 2024-05-01 has one. 76 tests, 39s.
    * **Nine plugins had no behavioural coverage whatsoever before this**: `BesselFilter` (present in the integration flow only to have its settings asserted - the event-finding step there deliberately passes an identity function instead, so `_apply_filter` was never called on real data), `WaveletFilter`, `BoundedBlockageFinder`, `ThresholdBlockageFinder`, `SQLitePeakDBLoader`, and the four fitters covered in the previous entry. `SQLiteEventLoader` was used as a parent but nothing asserted on what it returned, so a wrong padding or sample rate would have surfaced as a confusing fitter failure.
    * Checks are written for what each family's output is actually used for, not for "it did not raise": finders must locate exactly the planted events with boundaries that are ordered, in-bounds, non-overlapping and on the plantings; loaders must carry the exact keys `MetaEventLoader.get_event_generator`'s docstring specifies; writers are driven through a real chain and their output must pass SQLite integrity, hold the expected rows, **and read back through the matching loader**; database loaders must type every column they list and tell a valid filter query from an invalid one.
    * **`ThresholdBlockageFinder`'s `Threshold` is in sigma, not pA** - the second instance of this trap after `ClassicCUSUM`'s `Step Size`, and again invisible because nothing drove the plugin against data. It also needs 8.0 rather than 3.0: at 3.0 the detector trips on noise excursions, and although each is discarded as Too Short or Too Close, one landing beside a real event costs that event too - it found 4 of 5. 8.0 is clear of the noise and far below the 27 sigma the events reach.
    * **Finder boundaries are checked by containment, not equality.** `ClassicBlockageFinder` backtracks its boundaries to the local baseline and so reports a start a few samples before the planted index, while `ThresholdBlockageFinder` reports it exactly. Both are correct, so asserting exact positions would have failed a working plugin.
    * The writers' leak check is `os.unlink` on the output after `close_resources()`, which on Windows fails while any handle is open - stricter than `psutil.open_files()` and needing no new dependency. Both writers pass. Worth noting the trap it caught during development: `with sqlite3.connect(...)` commits a transaction but does **not** close the connection, so a test that inspects a writer's output that way looks like the leaking party itself. `tests/integration/flows` has the same pattern; harmless there only because pytest's `tmp_path` does not fail on cleanup.
    * Fixture parameters and discovery now live in `_recipes.py` rather than the conftest, so test modules import them as ordinary module-level imports; the three session-scoped datasets are built once for the whole suite.

* **Event fitters now have behavioural conformance tests: built like the app builds them, run against real data**
    * New `tests/unit/plugins/conformance/`, carrying a new `conformance` marker. `test_eventfitters.py` is parametrised over *discovered* `MetaEventFitter` subclasses, so a new fitter is covered the moment the file is dropped in - matching how `MainModel.populate_available_plugins` actually finds plugins, and how `test_plugin_compliance.py` already works. `_recipes.py` holds the settings recipes and is the piece a new plugin has to extend.
    * Four checks per fitter: it fits all 25 planted events rather than silently rejecting them; every metadata column it produces is declared in both `get_event_metadata_types()` and `get_event_metadata_units()`; `get_single_event_metadata()` returns float64 arrays of matching shape; and `reset_channel`/`close_resources` are safe after fitting, including twice.
    * This covers ground the existing tests structurally cannot. The six per-fitter unit suites build instances with `object.__new__` and attach `MagicMock()` loaders - deliberate, and good for testing a formula, but they never traverse `__init__` -> `apply_settings` -> `_finalize_initialization`, and they pass whether or not a real `MetaEventLoader` returns what the fitter assumes. Only `CUSUM` was previously driven through the real lifecycle at all, by `tests/integration/flows` and `tests/e2e/event_analysis`.
    * **The produced-columns check needed the base class distinguished from the subclass.** `MetaEventFitter.fit_events` injects `channel_id`/`event_id` into every event's metadata and `event_id`/`channel_id`/`level_id`/`levels_left` into every sublevel's, while `_define_metadata_types` declares only `event_id` (plus `start_time`, `num_sublevels`). So five of the seven fitters "produce undeclared columns" if you compare naively - the first version of this assertion was wrong. It now excludes the base-injected set and checks only what a subclass is responsible for.
    * **`ClassicCUSUM`'s `Step Size` is in sigma, not pA** - its `Units` entry says so, and its two siblings use pA. Against this signal (15 pA noise, 400 pA events, so about 27 sigma) the pA-scaled `100.0` that `CUSUM` and `IntraCUSUM` want detects nothing, and `ClassicCUSUM` rejected all 25 events as "Too Few Levels". It takes `3.0`. Nothing previously drove it against real data, so this never surfaced.
    * **`PeakFinder` and `Basic_PeakFinder` are skipped, with the reason recorded rather than the test quietly dropped.** `tests/synthetic_data`'s `_build_event_trace` plants one flat rectangular blockage per event and has no knob for intra-event structure, so a peak-based fitter has nothing to find: `PeakFinder` rejects all 25 as "No Peaks Found" and `Basic_PeakFinder` fits 1. That is a fixture limitation, not a plugin defect, and a pass or fail here would say nothing about either plugin until the fixture can plant resolvable peaks. Queued in `future_fixes.md`.
    * Contributor docs updated - `quality_control.rst` gains sections on the schema and conformance gates, and its pre-PR checklist now says `pytest tests/unit/plugins` (all three gates) rather than naming the compliance file alone.
    * Cost: 28 tests, 9.0s, mean 0.32s.

* **Fixed: `pytest tests/unit/views` failed on its own, and only passed as part of a full run**
    * Test modules import shared helpers absolutely (`tests.synthetic_data`, `tests.unit.views._qt_mocks`). None of those resolve unaided: there is no `__init__.py` under `tests/`, so pytest's prepend import mode inserts each test file's own directory rather than the repo root, bare `pytest` (unlike `python -m pytest`) does not add the working directory, and the editable install exposes only `poriscope`.
    * `tests/e2e/conftest.py` and `tests/integration/conftest.py` each carried their own copy of a `sys.path` shim for this. Those run for their own subtrees only, so the `tests/unit/views` imports worked purely because `tests/e2e` is collected first and had already patched `sys.path` - **`pytest tests/unit/views` alone died with `ModuleNotFoundError: No module named 'tests.unit'`**, taking 465 tests with it, while the full suite passed. Latent since the view tests gained `_qt_mocks.py`.
    * The shim now lives in the root `tests/conftest.py`, so it is independent of collection order. The other two copies are redundant but harmless and were left alone; both guard on the path not already being present.

* **Every plugin's `get_empty_settings()` is now checked against its own declared contract, and 21 violations of it are fixed**
    * New `tests/unit/plugins/test_settings_schema.py` parametrizes over all 24 concrete `BaseDataPlugin` subclasses and asserts two things per plugin: that every parameter carries the `Type` and `Value` keys `BaseDataPlugin.get_empty_settings`'s docstring calls required, and that any default a plugin *does* ship would survive that same plugin's own `_validate_param_types` / `_validate_param_ranges`. The validators are invoked rather than reimplemented, so the check cannot drift from the rules it enforces.
    * This was a real gap, not a restatement of `test_plugin_compliance.py`: that test never calls the plugin, the per-plugin unit tests build instances via `object.__new__` and inject settings directly, and the integration and e2e flows hand-write correct values over the defaults (`fset["Cutoff"]["Value"] = 200_000.0`) before anything validates them. 19 of the 48 new cases failed on first run.
    * **15 plugins omitted `"Value"` entirely** for parameters the caller must supply - every reader's `Input File`, both writers' `Output File`/`Experiment Name`/`Voltage`/`Membrane Thickness`/`Conductivity`, `SingleBinaryDecoder`'s `Sampling Rate`, and the CUSUM family's `Step Size`/`Rise Time`. Because `_validate_param_types` subscripts `val["Value"]` unguarded, an unfilled parameter raised `KeyError: 'Value'` rather than a message naming it. Most of these lived in the `Meta*` bases, whose own docstring examples already showed `"Value": None`, so five base-class edits fixed six readers and all four SQLite loaders/writers at once.
    * **11 `float` parameters shipped `int` defaults** - `Basic_PeakFinder`'s `Min Height`/`Min Prominence`/`Window Length`/`Width`/`Min Distance`/`Plateau Size`, `Sensitivity` on `CUSUM` and `ClassicCUSUM`, and `IntraCUSUM`'s two intraevent parameters. `_validate_param_types` uses `isinstance`, under which `isinstance(500, float)` is `False`, so each of these plugins rejected its own defaults.
    * **Behaviour change for programmatic callers only:** `apply_settings(get_empty_settings())` on an unfilled required parameter now raises `TypeError: Input File must have type str` instead of `KeyError: 'Value'`. Same failure, better message. The GUI is unaffected either way - `DictDialog` reads through `val.get("Value")` at all nine of its read sites, each guarded by `is not None`, and coerces every value through `self.params[key]["Type"](...)` on OK, which is precisely why both defect classes survived four test layers and a working application.
    * Two characterization tests asserted the old shape and are updated: `test_classic_cusum.py`'s `test_step_size_has_no_value_key` (now `test_step_size_has_explicit_none_value`) and `test_meta_database_loader.py`'s `test_base_impl_default`. The first described the quirk as "unlike every other setting dict in this codebase", which was itself inaccurate - 15 plugins shared it.
    * `Units` is tolerated rather than failed on. It is load-bearing - `SQLiteEventWriter` and `SQLiteDBWriter` both read `base_settings["Voltage"]["Units"]` - but appears in neither the docstring contract nor the `Setting` TypedDict, so the fix belongs to the contract, not to the plugins already relying on it.
    * Carries the existing `compliance` marker rather than introducing a new one, so `pytest.ini` is untouched.
    * Verified: 2,700 tests, 2,686 passed, 14 skipped, 0 failed; `pre-commit run --all-files` (ruff, mypy, pydoclint) passes.

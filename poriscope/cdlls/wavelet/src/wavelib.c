...
wt_object wt_init(wave_object wave,const char* method, int siglength,int J) {
	...
	if (J > MaxIter) {
		printf("\n Error 1 - The Signal Can only be iterated %d times using this wavelet. Exiting\n",MaxIter);
		exit(-1);
	}
...
wtree_object wtree_init(wave_object wave, int siglength,int J) {
	...
	MaxIter = wmaxiter(siglength, size);
	if (J > MaxIter) {
		printf("\n Error 2 - The Signal Can only be iterated %d times using this wavelet. Exiting\n", MaxIter);
		exit(-1);
	}
	temp = 1;
	elength = 0;
...
wpt_object wpt_init(wave_object wave, int siglength, int J) {
	...
	MaxIter = wmaxiter(siglength, size);
	if (J > MaxIter) {
		printf("\n Error 3 - The Signal Can only be iterated %d times using this wavelet. Exiting\n", MaxIter);
		exit(-1);
	}
	temp = 1;
	nodes = 0;
...
wt2_object wt2_init(wave_object wave, const char* method, int rows, int cols, int J) {
	...
	if (J > MaxIter) {
		printf("\n Error 4 - The Signal Can only be iterated %d times using this wavelet. Exiting\n", MaxIter);
		exit(-1);
	}

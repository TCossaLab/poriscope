...
- `peak_finding_pipeline.md` — the end-to-end `PeakFinder` pipeline: the per-event
  fitting hooks, the run-wide classifiers, and **every filter applied along the way**
  with the setting that governs it, plus a *Known gaps* list of things that read as live
  behaviour and are not. **Update it whenever a stage or a filter changes**, when a
  metadata field is added or its producer moves, or when a known gap is fixed.
- `fit_fallbacks.md` — every fallback path in `PeakFinder`'s shared double-Gaussian fit
  ...
  Fallback behaviour lives here; everything around the fit lives in
  `peak_finding_pipeline.md`.
